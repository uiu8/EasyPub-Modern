...
另外两行说明 **Level 正则的新旧值会改变章节数，但只在章节正则为旧 `LegacyPattern` 时**：
旧正则 + 开 + 旧 Level = 560，旧正则 + 开 + 新 Level = 567（差 7）；
而现代默认 + 开时，Level 新旧都给 567。**这个 7 的机制尚未查明**
（不是笔误，是矩阵里的实测值）—— 要动 §8.3 的升级表之前必须先弄清它，
否则"升级旧默认值"可能把 560 变成 567 而不是变成用户以为的那个数。
