...
| | 内容 |
|---|---|
| **真实字符串**（读 <c>SaveStateText</c> 属性） | `本次修复：改原文（会写回 TXT，改动前自动备份） · 已保存；返回后自动保存，未命名项目保留恢复快照` |
| **视觉模型转写** | `本次修改：改原文（会回到TXT，改动前自动保存）。已保存；回到后自动保存，未保存项目保存快照。` |

约 45 个字里错了 6 处：`修复→修改`、`写回→回到`、`备份→保存`、`返回→回到`、
`未命名→未保存`、`保留恢复快照→保存快照`；分隔符 ` · ` 读成 `。`。

**危险性不在于错，而在于错得像对的。** 每一个替换都是意思相近的常见词，
读起来通顺、方向上"没错"，只有逐字比对才看得出来。而 `改动前自动备份` 与
`改动前自动保存` 是**两个不同的承诺** —— 前者可核对，后者什么也没说。
**如果我拿那次转写去验收，就会"确认"一句界面并没有说的话。**

所以纪律是：

- **文案验收走代码**：`WorkbenchHarness.Line(window, 元素名)` 读控件的真实属性值，
  写成断言（见 `WorkbenchFooterTruthTests`）。这才是权威。
- **视觉只验收布局**：按钮是不是一行、有没有溢出、层级对不对、焦点在哪。
  这些它看得准，因为它不依赖逐字正确。
- 顺带一个更硬的理由：断言会在**以后每次改动**时继续生效，看图只看这一次。

**同一条道理在这个项目里出现过三次**（§11.1）：逐字段替换证明不了组合效应、
"这条测试本来就是红的"是猜测不是结论、**转写看起来对不等于字对**。
共同形状都是：**一个看起来合理的答案，不会自己声明它错了。**
