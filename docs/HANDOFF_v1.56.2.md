...
> **后续（2026-09-16）**：本文档写完后，Codex 又交付了 **v1.56.3**（侧栏导航文字溢出修复，`0a0bc70`）。
> **当前 HEAD 是 v1.56.3**，接手方已独立复核，最新状态见 `docs/RELEASE_v1.56.3.md`。
> 本文档 §3.3、§4、§5 已就地标注复核后哪些结论被修正、哪条待办被关闭。
...
> **后续修正（2026-09-16）**：前台限制并不是死路。**离线渲染**——自己 `new` 出控件、
> 手动 `Measure`/`Arrange`、直接改 `ScrollViewer.VerticalOffset`，再用 `RenderTargetBitmap`
> 存 PNG——完全不需要前台，连窗口都不用显示。§4 里"未复验"的那条就是靠这个关掉的，
> 工具见 `work/settings-scroll/`（Git 忽略）。只有**真实鼠标悬停、键盘焦点、系统级弹窗**
> 仍然需要前台，其余布局与滚动都能这样验。
...
| 转换设置面板滚动留白 | ✅ **已复验**（2026-09-16，离线渲染逐分类滚到底测量，结论见 `RELEASE_v1.56.3.md`） |

测试：Core **331** + Desktop **120**。（Desktop 在 v1.56.3 增至 **123**，新增 3 项真实布局断言。）
...
1. ~~手测转换设置面板滚到底的留白~~ —— **2026-09-16 已关闭**：不需要前台，离线渲染逐分类
   滚到底测量，余量 50 / 不需滚动 / 50 / 不需滚动 / 69.6，底栏顶 = 内容区底，不重叠。
   窄窗口 + 深色同样通过。工具 `work/settings-scroll/`，结论见 `RELEASE_v1.56.3.md`。
