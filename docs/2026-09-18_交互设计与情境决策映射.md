...
工作台顶部增加一条**始终可见**的事实状态条（替代现在 `ChapterWorkbench.cs:98` 那句静态说明）：
...
| 行 | 取值 | 数据来源 |
|---|---|---|
| **参考目录** | 未加载 ／ 已加载「书名」· 480 章 · 未确认 ／ 已确认使用 | `ReferenceCatalogInput.Load`（`ChapterAutoRepair.cs:191`） |
| **当前章节树** | 本地识别 ／ 目录校验后 ／ 目录校验后 + 手工修改 N 项 ／ 原文迁移后 | **持久化的 provenance**（`ChapterTreePlan.ReferenceCatalog`）+ `manual` 条目计数。<br>⚠️ **不得**仅凭当前 `RecognitionSource` 分布反推——手工编辑会覆盖 reference 标记（§8.2.2b） |
| **本次检查依据** | 本地规则 ／ 按参考目录核对 | 用户选择（或按 §3.4 推荐后确认） |
...
**一个 Intent 可以有两个 CTA，但不能有两种语义。** `ChapterIssueAction` 的现有规则（`ChapterIssueAction.cs:20-29`）是现有实现里最正确的一份，应当成为 `IssueResolutionPolicy` 的**内部判断逻辑来源**，另两处改为委托给它——但委托的是**语义判断**，不是字符串。
